...
namespace midterm_pos_terminal
{
    public class ConesSoftServe : ItemProperties
    {
        ...
        //EDIT FOR SOFTSERVE PURPOSES !!!
        public class MenuTextFile
        {
            public static void WriteFlavorMenu(List<string> flavors)
            {
                StreamWriter writer = new StreamWriter("../../../Flavors.txt");

                foreach (string flavor in flavors)
                {
                    writer.WriteLine(flavors);
                }

                writer.Close();
            }
            public static List<ConesSoftServe> GetFlavorMenu()
            {
                List<ConesSoftServe> flavors = new List<ConesSoftServe>();
                StreamReader reader = new StreamReader("../../../Flavors.txt");
                string line = reader.ReadLine();
                while (line != null)
                {
                    string[] flavor = line.Split('|');

                    flavors.Add(new ConesSoftServe(flavor[0], double.Parse(flavor[1])));
